...
export default function AdminDashboard() {
    ...
    const dashboardStats = [
        { label: t('totalUsers'), value: stats.totalUsers, icon: Users, color: 'bg-blue-500', change: `+${stats.newUsersToday} ${t('today')}` },
        { label: t('totalPosts'), value: stats.totalPosts, icon: FileText, color: 'bg-green-500', change: `+${stats.newPostsToday} ${t('today')}` },
        { label: t('pendingReports'), value: stats.flaggedPosts, icon: Flag, color: 'bg-red-500', change: t('needsReview') },
        { label: t('activeUsers'), value: stats.activeUsers, icon: TrendingUp, color: 'bg-purple-500', change: t('liveNow') },
    ];
    ...
    return (
        <div className="space-y-6 pb-12">
            ...
            <div className="bg-gradient-to-r from-primary to-primary-dark rounded-[2rem] p-10 text-white shadow-2xl relative overflow-hidden border border-white/10">
                <h1 className="text-5xl font-black mb-2 uppercase tracking-tighter">{t('adminCommandCenter')}</h1>
                <p className="text-blue-100 italic font-medium">"{t('helpingReconnect')}"</p>
                    ...
                    return (
                        <div key={index} className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-8 hover:scale-[1.02] transition-all duration-300 border border-gray-100 dark:border-gray-700">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <span className="bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest">{t('global')}</span>
                            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-8 border border-gray-100 dark:border-gray-700">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h3 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">{t('weeklyActivity')}</h3>
                            <p className="text-xs text-gray-400 dark:text-gray-500 font-bold uppercase tracking-widest mt-1">{t('itemFlowAnalytics')}</p>
                        </div>
                        <div className="flex gap-4">
                            <div className="flex items-center gap-2 bg-red-50 dark:bg-red-900/10 px-3 py-1.5 rounded-xl border border-red-100 dark:border-red-900/20">
                                ...
                                <span className="text-[10px] font-black text-red-600 dark:text-red-400 uppercase tracking-widest">{t('lost')}</span>
                            </div>
                            <div className="flex items-center gap-2 bg-green-50 dark:bg-green-900/10 px-3 py-1.5 rounded-xl border border-green-100 dark:border-green-900/20">
                                ...
                                <span className="text-[10px] font-black text-green-600 dark:text-green-400 uppercase tracking-widest">{t('found')}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-8 border border-gray-100 dark:border-gray-700">
                    <h3 className="text-xl font-black text-gray-900 dark:text-white mb-8 uppercase tracking-tight">{t('distribution')}</h3>
                    <div className="flex flex-col h-full justify-center space-y-10 pb-8">
                        <div>
                            <div className="flex justify-between mb-3 text-[10px] font-black uppercase tracking-widest">
                                <span className="text-gray-400">{t('lostItems')}</span>
                        ...
                        <div>
                            <div className="flex justify-between mb-3 text-[10px] font-black uppercase tracking-widest">
                                <span className="text-gray-400">{t('foundItems')}</span>
                        ...
                        <div className="pt-6 border-t border-gray-100 dark:border-gray-700 text-center">
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">{t('successResolution')}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-4">
                    <h3 className="text-xl font-black text-gray-900 dark:text-white mb-4 uppercase tracking-tighter">{t('tools')}</h3>
                    <div className="grid grid-cols-1 gap-4">
                        {[
                            { to: '/admin/posts', icon: FileText, label: t('managePosts'), iconColor: 'text-primary', bgColor: 'bg-blue-100 dark:bg-blue-900/30' },
                            { to: '/admin/users', icon: Users, label: t('userDirectory'), iconColor: 'text-purple-600', bgColor: 'bg-purple-100 dark:bg-purple-900/30' },
                            { to: '/admin/claims', icon: Shield, label: t('claimsApproval'), iconColor: 'text-green-600', bgColor: 'bg-green-100 dark:bg-green-900/30' },
                            { to: '/admin/reports', icon: Flag, label: t('infractionReports'), iconColor: 'text-red-600', bgColor: 'bg-red-100 dark:bg-red-900/30', count: stats.flaggedPosts }
                        ].map((action, idx) => (
                            <Link
                                key={idx}
                                to={action.to}
                            ...
                            >
                                ...
                                <div className="flex-1">
                                    ...
                                    {action.count > 0 && (
                                        <p className="text-[10px] text-red-500 font-bold uppercase tracking-widest">{action.count} {t('pendingReview')}</p>
                                    )}
                                </div>
                ...
                <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-3xl shadow-xl overflow-hidden border border-gray-100 dark:border-gray-700">
                    <div className="p-8 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-900/20">
                        <div>
                            <h3 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">{t('recentActivity')}</h3>
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">{t('liveFeedContributions')}</p>
                        </div>
                        <Link to="/admin/posts" className="px-5 py-2.5 bg-primary text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-primary-dark transition shadow-lg shadow-primary/20">{t('viewAll')}</Link>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
                            <thead className="bg-gray-50 dark:bg-gray-900/50">
                                <tr>
                                    <th className="px-8 py-4 text-left text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em]">{t('itemDetails')}</th>
                                    <th className="px-8 py-4 text-left text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em]">{t('classification')}</th>
                                    <th className="px-8 py-4 text-left text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em]">{t('verification')}</th>
                                    <th className="px-8 py-4 text-right text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em]">{t('timeline')}</th>
                                </tr>
                            </thead>
                                    ...
                                    recentPosts.map((post) => (
                                        <tr key={post.id} className="hover:bg-primary/5 dark:hover:bg-primary/5 transition duration-150 group">
                                            <td className="px-8 py-5 whitespace-nowrap">
                                                ...
                                                <div className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mt-0.5">{t('by', { defaultValue: 'by' })} {post.creatorName || t('anonymous')}</div>
                                            </td>
                                            <td className="px-8 py-5 whitespace-nowrap">
                                            ...
                                            <td className="px-8 py-5 whitespace-nowrap">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[10px] font-black text-gray-700 dark:text-gray-300 uppercase tracking-widest">{t(post.status || 'Active', { defaultValue: post.status || 'Active' })}</span>
                                                </div>
                                            </td>
