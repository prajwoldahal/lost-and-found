    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-5xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">{t('moderationCenter')}</h1>
                    <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px] mt-2">{t('investigateResolveReports')}</p>
                </div>
                <div className="flex items-center gap-4">
                    <div className="bg-gray-100 dark:bg-gray-800 px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col items-center">
                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t('reportsCount')}</span>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-8 border border-gray-100 dark:border-gray-700">
                <div className="relative max-w-xs">
                    ...
                    >
                        <option value="all">{t('comprehensiveAnalysis')}</option>
                        <option value="pending">{t('pendingInvestigation')}</option>
                        <option value="reviewed">{t('underReview')}</option>
            ...
            <div className="space-y-6">
                {filteredReports.length > 0 ? filteredReports.map((report) => (
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t('informant')}</span>
                                            <span className="text-sm font-bold text-gray-700 dark:text-gray-200 uppercase tracking-tight">{report.reporterName || t('anonymous')}</span>
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t('primaryReason')}</span>
                                    ...
                                    <div className="bg-gray-50 dark:bg-gray-900/50 p-6 rounded-3xl border border-gray-100 dark:border-gray-700">
                                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">{t('detailedDescription')}</span>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-[3rem] shadow-2xl p-24 text-center border border-gray-100 dark:border-gray-700">
                        ...
                        <h3 className="text-3xl font-black text-gray-900 dark:text-white uppercase tracking-tighter mb-4">{t('clearSkies')}</h3>
                        <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">{t('allReportsAdjudicated')}</p>
                    </div>
                )}
            </div>
        </div>
